...
@Component({
  selector: 'app-trajet-checker',
  standalone: true,
  templateUrl: './trajet-checker.component.html',
  styleUrl: './trajet-checker.component.scss',
  imports: [
        CommonModule,
        MatTableModule,
        MatButtonModule,
  ...
  export class TrajetCheckerComponent implements OnInit {
    trajets: Trajet[] = [];
    ...
    addPassager(trajet: Trajet) {
      ...
      if (axeId) {
        this.localiteService.loadByAxe(axeId).subscribe({
          ...
          error: (err) => {
            console.error('[TrajetCheckerComponent] failed to load localites by axe', err);
